...
@AsyncAPI(asyncapi = "2.0.0",
    ...
    info = @Info(title = "Streetlights API",
        version = "1.0.0",
        description = "The Smartylighting Streetlights API allows you to remotely manage the city lights. ### Check out its awesome features: * TurnOnOff a specific streetlight on/off * Dim a specific streetlight * Receive real-time information about environmental lighting conditions",
    ...
    servers = { @Server(url = "api.streetlights.smartylighting.com:{port}",
        ...
        security = { @SecurityRequirement(name = "apiKey"), @SecurityRequirement(name = "supportedOauthFlows",
            values = { "streetlights:on", "streetlights:off", "streetlights:dim" }),
            @SecurityRequirement(name = "openIdConnectWellKnown") }) },
    tags = { @Tag(name = "streetlights") })
@SecuritySchemes(values = { @SecurityScheme(name = "apiKey",
    type = SecuritySchemeType.APIKEY,
    in = SecuritySchemeIn.USER,
    description = "Provide your API key as the user and leave the password empty."), @SecurityScheme(name = "supportedOauthFlows",
    type = SecuritySchemeType.OAUTH2,
    in = SecuritySchemeIn.DEFAULT,
    description = "Flows to support OAuth 2.0",
    flows = @OAuthFlows(implicit = @OAuthFlow(authorizationUrl = "https://authserver.example/auth",
        scopes = { @OAuthScope(name = "streetlights:on",
            description = "Ability to switch lights on"), @OAuthScope(name = "streetlights:off",
            description = "Ability to switch lights off"), @OAuthScope(name = "streetlights:dim",
            description = "Ability to dim the lights") }),
        password = @OAuthFlow(tokenUrl = "https://authserver.example/token",
            scopes = { @OAuthScope(name = "streetlights:on",
                description = "Ability to switch lights on"), @OAuthScope(name = "streetlights:off",
                description = "Ability to switch lights off"), @OAuthScope(name = "streetlights:dim",
                description = "Ability to dim the lights") }),
        clientCredentials = @OAuthFlow(tokenUrl = "https://authserver.example/token",
            scopes = { @OAuthScope(name = "streetlights:on",
                description = "Ability to switch lights on"), @OAuthScope(name = "streetlights:off",
                description = "Ability to switch lights off"), @OAuthScope(name = "streetlights:dim",
                description = "Ability to dim the lights") }),
        authorizationCode = @OAuthFlow(authorizationUrl = "https://authserver.example/auth",
            tokenUrl = "https://authserver.example/token",
            refreshUrl = "https://authserver.example/refresh",
            scopes = { @OAuthScope(name = "streetlights:on",
                description = "Ability to switch lights on"), @OAuthScope(name = "streetlights:off",
                description = "Ability to switch lights off"), @OAuthScope(name = "streetlights:dim",
                description = "Ability to dim the lights") }))), @SecurityScheme(name = "openIdConnectWellKnown",
